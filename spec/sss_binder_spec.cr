...
describe CrystalClevisZfs::SssBinder do
  describe "round-trip via MockTangClient (no network)" do
    it "binds with K=2/N=3 and recovers with any 2 of 3 Tangs" do
      ...
      jwe = CrystalClevisZfs::SssBinder.bind(payload, tangs, threshold: 2)
      jwe.split('.').size.should eq(5)

      recovered = CrystalClevisZfs::SssBinder.recover(jwe, &mock_tang_factory)
    ...
    it "still recovers when 1 Tang is unreachable (K=2/N=3)" do
      ...
      tangs = [
        ...
        MockTangClient.new("http://tang-2.example"),
        MockTangClient.new("http://tang-3.example"),
      ] of CrystalClevisZfs::TangClient
    ...
    it "fails when too many Tangs are down (K=2/N=3, 2 down)" do
      ...
      jwe = CrystalClevisZfs::SssBinder.bind("should fail", tangs, threshold: 2)

      MOCK_TANG_REGISTRY.delete("http://tang-1.example")
      MOCK_TANG_REGISTRY.delete("http://tang-2.example")

      expect_raises(CrystalClevisZfs::SssBinder::Error, /shares/) do
        CrystalClevisZfs::SssBinder.recover(jwe, &mock_tang_factory)
      end
    end
    ...
    it "round-trips K=N=3 (all required, no panne tolerated)" do
      ...
      jwe = CrystalClevisZfs::SssBinder.bind("strict", tangs, threshold: 3)
      recovered = CrystalClevisZfs::SssBinder.recover(jwe, &mock_tang_factory)
    ...
    it "round-trips K=1/N=1 (degenerate, equivalent to plain TangClient)" do
      ...
      jwe = CrystalClevisZfs::SssBinder.bind("solo", tangs, threshold: 1)
      recovered = CrystalClevisZfs::SssBinder.recover(jwe, &mock_tang_factory)
    ...
    it "round-trips a 64-byte GELI keyfile" do
      ...
      keyfile = CrystalClevisZfs::Zfs.random_key_hex.to_slice
      jwe = CrystalClevisZfs::SssBinder.bind(keyfile, tangs, threshold: 2)
      recovered = CrystalClevisZfs::SssBinder.recover(jwe, &mock_tang_factory)
  ...
  describe "is_sss?" do
    it "detects SSS-flavored JWEs" do
      ...
      jwe = CrystalClevisZfs::SssBinder.bind("payload", tangs, threshold: 1)
      CrystalClevisZfs::SssBinder.is_sss?(jwe).should be_true
    end
    ...
    it "returns false for a plain Tang JWE (no SSS)" do
      ...
      CrystalClevisZfs::SssBinder.is_sss?(jwe).should be_false
    end

    it "returns false for malformed JWEs" do
      CrystalClevisZfs::SssBinder.is_sss?("not.a.jwe").should be_false
    end
  end
  ...
  describe "validation" do
    it "rejects threshold > number of Tangs" do
      ...
      expect_raises(CrystalClevisZfs::SssBinder::Error, /threshold/) do
        CrystalClevisZfs::SssBinder.bind("p", tangs, threshold: 2)
      end
    end

    it "rejects an empty Tang list" do
      empty = [] of CrystalClevisZfs::TangClient
      expect_raises(CrystalClevisZfs::SssBinder::Error, /required/) do
        CrystalClevisZfs::SssBinder.bind("p", empty, threshold: 1)
      end
    end
  end
end
